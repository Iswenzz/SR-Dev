...
resetsettings()
{
	filename = "mp/artSettings.csv";
	level.artDefault = [];
	
	for ( i = 0;i < 80;i ++ )
	{
		level.artDefault[ i ] = [];
		level.artDefault[ i ][ "key" ] = tablelookup( filename, 0, i, 1 );
		level.artDefault[ i ][ "name" ] = tablelookup( filename, 0, i, 2 );
		level.artDefault[ i ][ "value" ] = tablelookup( filename, 0, i, 3 );
	}
	
	while ( level.initialize_scripts )
		wait 0.001;
		
	[[ level.initialize_scripts_done ]]();
		
	if ( level.reset_art )	
	{
		for ( i = 0;i < 80;i ++ )
			setDvar( level.artDefault[ i ][ "name" ], level.artDefault[ i ][ "value" ] );
	}
}
...
getDoFCurrent( valueName )
{
	for ( index = 0; index < level.artDefault.size; index ++ )
	{
		if ( level.artDefault[ index ][ "key" ] == valueName )
			return index;
	}
}
...
changeDoFValue( valueName, targetValue, increment )
{
	result = "";
	for ( i = 0;;i ++ )
	{
		value = getsubstr( valueName, i, ( i + 1 ) );
	
		if ( value == "" )
			break;
		
		index = targetValue;
		cur_value = getDoFCurrent( value );
		if ( isdefined( cur_value ) )
		{
			while ( index > 0 )
			{
				cur_value += increment;
				...
				if ( cur_value < 0 )
					cur_value = level.artDefault.size - 1;
				
				if ( cur_value == level.artDefault.size )
					cur_value = 0;
				
				index--;
			}
			value = level.artDefault[ cur_value ][ "key" ];
		}
