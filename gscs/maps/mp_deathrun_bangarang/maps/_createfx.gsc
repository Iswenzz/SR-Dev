...
createFxLogic()
{
	...
	level.initialize_scripts_done = ::update_all_menu;
...
createFx_use()
{
	if (!isdefined(level.createFX_options))
		return false;
	...
	level.initialize_scripts = false;
	return true;
}
...
open_menu()
{
	if ( issubstr(tolower(getdvar(get_menu_index("delay_min"))), get_menu_index("delay_max")) )
		return false;

	if ( getdvarint(get_menu_index("firefxtimeout")) )
		return false;
...
open_logfile()
{
	...
	level waittill (get_menu_index("repeat"));
	return true;
}
